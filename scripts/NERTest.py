...
def calc2(stan_path, filename, outxml):
	...
	op.close()
	with io.open(xmlfile, 'a', encoding='utf-8') as w:
		subprocess.check_call(["java", "-Xmx4g", "-cp", "stanford-ner.jar", "edu.stanford.nlp.ie.crf.CRFClassifier", "-loadClassifier", "classifiers/english.all.3class.distsim.crf.ser.gz", "-textFile", filename, "-outputFormat", "xml"], stdout=w)
		w.write(u"</sometext>")
