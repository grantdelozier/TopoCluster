...
###Main Method###
def calc(f, statistic, dtbl, gtbl, conn_info, outf, out_tbl, kern_dist, kerntype, traintype, listuse, whitelist_file, grid_freq_min, cores, include_zero, proc_id):
    ...
    if statistic.lower() == "gi":
        ...
        cur.execute("CREATE TABLE IF NOT EXISTS %s (gid varchar(20), word varchar(50), stat float);" % (out_tbl_gi, ))
        ...
        with open("tmp_gids.txt", 'r') as w:
            for line in w:
                row = line.strip().split('\t')
                if str(proc_id) == row[0]:
        ...
        GiCalcs([gtbl, dtbl, id_list, kern_dist, kerntype, conn_info, docDict, word_totals, F_All, out_tbl_gi, include_zero])
