...
def calc(f, statistic, dtbl, gtbl, conn_info, outf, out_tbl, kern_dist, kerntype, traintype, listuse, whitelist_file, grid_freq_min, cores, include_zero):
    ...
    if statistic.lower() == "zavg":
        ...
        cur.execute("CREATE TABLE IF NOT EXISTS %s (gid varchar(20), word varchar(50), stat float);" % (out_tbl_zavg, ))
    ...
    if statistic.lower() == "gi":
        ...
        k = 0
        with open('tmp_gids.txt', 'w') as w:
            for chunk in id_lists:
                k += 1
                for i in chunk:
                    w.write(k +'\t' + i + '\r\n')

        print "Done writing tmp_gids.txt"
