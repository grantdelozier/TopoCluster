...
def getAllDicts(indomain_tab_results, outdomain_tab_results, weight, lamb_dict, percentile):
	for in_corp_lamb in lamb_dict.keys():
		...
		out_corp_lamb = 1.0 - float(in_corp_lamb)
		...
		lamb_dict[in_corp_lamb] = merge1([adict, adict2])
	return lamb_dict
...
def VectorSum(wordref, toporef, total_topo, cur, lat_long_lookup, percentile, window, stopwords, place_name_weight, xml,
 country_tbl, region_tbl, state_tbl, geonames_tbl, tst_tbl, country_alt, region_alt, state_alt, pplc_alt, in_domain_stat_tbl, lamb_dict_total):
	...
	for j in toporef:
		...
		for word in contextlist:
			if word not in stopwords:
				...
				if len(table) > 0:
					...
					#Removing this part for efficiency, may want to return for debug
		...
		#lamb_dict = merge2(lamb_dict_list)
		#totaltime_dict_adding += datetime.datetime.now() - start_dict_adding
		...
		for in_domain_lamb in lamb_dict:
