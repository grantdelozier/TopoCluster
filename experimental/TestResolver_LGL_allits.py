...
def calc(in_domain_stat_tbl, out_domain_stat_tbl, test_xml, conn_info, gtbl, window, percentile, place_name_weight, country_tbl, 
	region_tbl, state_tbl, geonames_tbl, tst_tbl, results_file):	
...
def merge1(dicts, defaultdict=defaultdict):
...
def merge2(dicts, defaultdict=defaultdict):
	lamb_dict = {}
	for lam in dicts[0]:
		#print lam
		merged = defaultdict(float)
		for d in dicts:
			...
			for k in d[lam]:
				...
				if k == 42031:
					...
					print k, d[lam][k]
				merged[k] += d[lam][k]
		lamb_dict[lam] = merged
	return lamb_dict

def merge3(alist):
	lamb_dict = {}
	print "Number words: ", len(alist)
	#print alist
	for dic in alist:
		#print dic.keys()
		for lam in dic:
			...
			if lam == 0.0:
				print "!!!Here!!!"
				print sorted(dic[lam].items(), key=itemgetter(1), reverse=True)[:10]
			if lam not in lamb_dict:
				lamb_dict[lam] = {}
			for k in dic[lam]:
				lamb_dict[lam][k] = lamb_dict[lam].get(k, 0.0) + float(dic[lam][k])
				if float(dic[lam][k]) > 1.0:
					print "Found greater than 1.0"
					print lam
					print k
					print dic[lam][k]
				if k == 42031:
					pass
					#print "Key 42031"
					#print k, dic[lam][str(k)]
					#print k, dic[lam][k]
					#print lam, lamb_dict[lam][k]
	sys.exit()
	return lamb_dict 
...
def mergedicts(d1, d2):
	...
	for lam in d2:
		...
		for k in d2[lam]:
			...
			#print type(d1[lam])
			#print len(d1[lam])
			d3[lam][k] = (d3[lam].get(k, 0.0) + d2[lam][k])
	for lam in d1:
		if lam not in d3:
			d3[lam] = {}
		for k in d1[lam]:
			d3[lam][k] = (d3[lam].get(k, 0.0)  + d1[lam][k])
	return d3


#This function iterates though lambdas assigned to two corpora and calculates gi* vectors at each lambda
def getAllDicts(indomain_tab_results, outdomain_tab_results, weight, lamb_dict, percentile):
	for in_corp_lamb in lamb_dict.keys():
...
def VectorSum(wordref, toporef, total_topo, cur, lat_long_lookup, percentile, window, stopwords, place_name_weight, xml,
 country_tbl, region_tbl, state_tbl, geonames_tbl, tst_tbl, country_alt, region_alt, state_alt, pplc_alt, in_domain_stat_tbl, lamb_dict_total):
	...
	start_time = datetime.datetime.now()
	xml = xml[:-4]
	totaltime_word_query = (datetime.datetime.now() - datetime.datetime.now())
	totaltime_dict_adding = (datetime.datetime.now() - datetime.datetime.now())
	totaltime_gazet_query = (datetime.datetime.now() - datetime.datetime.now())
	for j in toporef:
		xml = toporef[j][3]
		wid = toporef[j][4]
		...
		#Reset lambda dict for each toponym
		lamb_dict = {}
		lamb_dict_list = {}
		...
		#getcontext().prec = 2
		for i in range(0, 10+1):
			lamb_dict[round((Decimal(i)*Decimal(0.1)), 2)] = {}
			totaldict[round((Decimal(i)*Decimal(0.1)), 2)] = Counter()
			lamb_dict_list[round((Decimal(i)*Decimal(0.1)), 2)] = []
		...
		for word in contextlist:
			if word not in stopwords:
				...
				if len(table) > 0:
					...
					start_word_query = datetime.datetime.now()
					cur.execute(SQL, (word, ))
					#print word
					if word in topotokens:
						weight = place_name_weight
					else: weight = 1.0
					outdomain_tab_results = cur.fetchall()
					cur.execute(SQL2, (word, ))
					totaltime_word_query += (datetime.datetime.now() - start_word_query)
					indomain_tab_results = cur.fetchall()
					...
					for idl in defmergeddict:
						...
						#totaldict[idl] += Counter(subset_ranked)

					totaltime_dict_adding += datetime.datetime.now() - start_dict_adding

					#subset_ranked = dict(ranked_fetch[:int(len(ranked_fetch)*percentile)])

					#subset_ranked = ranked_fetch
		...
		#Something very wrong with merge2, is producing incorrect output
		#print len(lamb_dict_list)
		#lamb_dict = merge3(lamb_dict_list)
		#print "!! Is it working here !!"
		#print contrib_dict_lambs[0.0][42031]
		#print lamb_dict[0.0][42031]
		
		print "Time for word queries: ", totaltime_word_query
		#print "Time for Dict adding: ", totaltime_dict_adding
		start_gazet_query = datetime.datetime.now()
		#for in_domain_lamb in sorted(totaldict.keys()):
